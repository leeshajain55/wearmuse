...
export default function AdminPage() {
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      if (editingProduct) { await api.put(`/products/${editingProduct._id}`, data); toast.success('Product updated!'); }
  ...
  const handleSaveCat = async () => {
    ...
    try {
      if (editingCat) { await api.put(`/categories/${editingCat._id}`, catForm); toast.success('Updated!'); }
      else { await api.post('/categories', catForm); toast.success('Created!'); }
  ...
  const handleUpdateOrderStatus = async (orderId, status) => {
    try { await api.put(`/orders/${orderId}/status`, { status }); toast.success(`Status → ${status}`); fetchAll(); }
    catch { toast.error('Failed'); }
  };
